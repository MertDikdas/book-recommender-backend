...
class BookService:
    ...
    def get_book(self, book_id: int) -> BookEntity:
        with self.uow as uow:
            return self.uow.books.get_by_id(book_id)
    ...
    def search_books(self, query: str) -> list[BookEntity]:
        with self.uow as uow:
            ...
            if not query:
                ...
                return []
            return self.uow.books.search_books(query)
    ...
    def create_comment(self, book_id:int , username:str, comment_text:str) -> CommentEntity:
        with self.uow as uow:
            ...
            if not user:
                return None
            comment = self.uow.books.add_comment(book.id,user.id,comment_text)
            uow.commit()
            return comment
    ...
    def get_comments_by_book_id(self, book_id:int) -> list[CommentEntity]:
        with self.uow as uow:
            ...
            if not book:
                return None
            comments = self.uow.books.get_comments_by_book_id(book_id)
            return comments
    
    def delete_comment_by_id(self, comment_id:int):
        with self.uow as uow:
            try:
                self.uow.books.delete_comment_by_id(comment_id)
                uow.commit()
